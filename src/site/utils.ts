...
export const siteUrl = (path = "", basePath = import.meta.env.BASE_URL) => {
...
export const imagePreviewUrl = (filename: string, width: number) => {
  return imageUrl(`previews/${filename.replace(/\.[^.]+$/, "")}-${width}.webp`);
};

export const imageSources = (filename: string) => {
...
export const pageSlug = (pathname: string, basePath: string) => {
